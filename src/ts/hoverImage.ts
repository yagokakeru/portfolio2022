...
import vertexShader from '../shaders/hoverImageVert.glsl';
import fragmentShader from '../shaders/hoverImageFrag.glsl';
...
export class hoverImage {
    ...
    constructor(canvasTarget: HTMLElement, itemsWrapper: HTMLElement, targetItems: NodeListOf<Element>) {
    ...
    setRender() {
        ...
        this.renderer.domElement.classList.add('header_hamburger_menu_webgl');
        if(this.canvasTarget !== null){
    ...
    setEventListeners() {
        if(this.items !== undefined && this.itemsWrapper !== null && this.canvasTarget !== null) {
            this.items.forEach((item, index) => {
    ...
    onMouseMove(event: MouseEvent) {
        let mouseX = (event.clientX / this.windowSize.width) * 2 - 1;
